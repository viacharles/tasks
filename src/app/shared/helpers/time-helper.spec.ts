...
describe('TimeHelper', () => {
  ...
  it('should format a date to Tw format', () => {
    ...
    expect(formatted).toBe(moment(date).locale('zh-tw').format('YYYY-MM-DD'));
  });

  it('should format a date to En format', () => {
    const date = '2024-08-01';
    const formatted = TimeHelper.formatDateEn(date);
    expect(formatted).toBe(moment(date).locale('en').format('YYYY-MM-DD'));
  });
  ...
  it('should return specific date added by amount', () => {
    ...
    const expectedDate = moment(date).add(amount, unit).toISOString();
  ...
  it('should return boundary date for a given unit', () => {
    ...
    const expectedStart = moment(date).add(amount, unit).startOf(unit).toISOString();
    const expectedEnd = moment(date).add(amount, unit).endOf(unit).toISOString();
  ...
  it('should calculate the offset between two dates', () => {
    ...
    const expectedOffset = moment(end).diff(moment(start), unit) + 1; // Adding 1 to include start day
